...
function renderReport(report) {
  ...
  pageResults.innerHTML = report.pages.map(page => {
    ...
    return `<article class="page-result"><div class="page-head"><div><h3>${escapeHtml(page.title || 'Untitled page')}</h3><div class="url" title="${escapeHtml(page.url)}">${escapeHtml(page.url)}</div></div><span class="pill ${page.status < 400 ? 'ready' : ''}">HTTP ${escapeHtml(page.status)}</span></div><div class="checks">${page.checks.map(check => `<div class="check ${check.passed ? '' : 'fail'}"><span class="mark">${check.passed ? '✓' : '×'}</span><div><b>${escapeHtml(check.name)}</b><small>${escapeHtml(check.detail)}${check.passed ? '' : ` · ${escapeHtml(check.severity)}`}</small></div></div>`).join('')}</div>${apiIssues.length ? `<h3>API issues</h3>${apiIssues.map(call => `<div class="check fail"><span class="mark">×</span><div><b>${escapeHtml(call.method)} · HTTP ${escapeHtml(call.status)}</b><small>${escapeHtml(call.url)}</small></div></div>`).join('')}` : ''}</article>`;
  }).join('');
  if ((report.duplicateUrls || []).length) pageResults.innerHTML += `<article class="page-result"><h3>Duplicate internal URLs</h3>${report.duplicateUrls.map(item => `<div class="check fail"><span class="mark">×</span><div><b>${escapeHtml(item.occurrences)} occurrences</b><small>${escapeHtml(item.url)} · found on ${escapeHtml(item.foundOn.join(', '))}</small></div></div>`).join('')}</article>`;
